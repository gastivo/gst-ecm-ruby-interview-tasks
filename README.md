...
interview tasks for software developer jobs at Gastivo. Interview tasks are
...
Make sure you have all the development dependencies installed:
  ...
  - A recent Ruby version (`3.4.x` is recommended)
...
```
git clone git@github.com:gastivo/contact_book.git
```
...
If all is fine, you should be able to run all tests with `bundle exec rspec`.
...
[shakapacker](https://github.com/shakacode/shakapacker) gem to compile and
serve the frontend code (JavaScript and CSS).
...
### Start the app in development mode
  ...
  - Run `./bin/dev` to start the app and its dependencies
  - Your app should now be running on `http://localhost:3000`

### Run tests

  - Run `bundle exec rspec` to run only Ruby tests
  - Run `npm run test` to run only JavaScript tests
...
To make sure your code formatting follows the project conventions, you can:
  ...
  - Run `bundle exec rubocop` to check for code formatting issues
  - Run `bundle exec rubocop --autocorrect-all` to automatically fix formatting issues (when possible)
